...
export default function App() {
  ...
  useEffect(()=>{Promise.all([api<PublicConfig>('/api/config').then(setConfig),refresh()]).catch(e=>setError(e.message));},[]);
  useEffect(()=>{if(!active)return; const timer=setInterval(()=>void refresh().catch(e=>setError(e.message)),1800);return()=>clearInterval(timer);},[active?.id]);
  ...
  async function start(body:Record<string,unknown>){
    ...
    try{const r=await api<Investigation>('/api/runs',{method:'POST',body:JSON.stringify({...body,locale})});setRuns(prev=>[r,...prev]);navigate(r.id);}
  ...
  async function stop(){if(!run)return;setBusy(true);try{await api(`/api/runs/${run.id}/stop`,{method:'POST'});await refresh();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
  ...
  return <div className="app-shell">
    ...
    <main id="main">
      {!selected ? <>
        ...
        {run.status==='running'?<div className="running-panel"><span className="large-icon"><LoaderCircle aria-hidden="true" className="spin" size={30}/></span><p className="eyebrow">{t("正在核对公开来源")}</p><h1>{t("线索正在变成依据。")}</h1><p>{t("我们会按已有证据决定下一步。你可以留在这里，也可以稍后从记录继续查看。")}</p><div role="status" className="current-action">{run.actions.at(-1)?.reason||t("正在等待模型开始调查；尚未查询网页或链上资料。")}</div><button className="secondary" disabled={busy} onClick={()=>void stop()}><Square aria-hidden="true" size={14}/>{t("停止本次核验")}</button></div>:run.report?<>
          ...
          <div className="report-actions"><button className="secondary" onClick={()=>void copy()}><Copy aria-hidden="true" size={16}/>{t("复制摘要")}</button><a className="secondary" href={`/api/runs/${run.id}/export?locale=${locale}`} download><Download aria-hidden="true" size={16}/>{t("下载报告")}</a>{run.input.flow&&<button className="text-button" disabled={busy||!!active} onClick={()=>void start({previousId:run.id})}>{t("重新核验")}<ArrowRight aria-hidden="true" size={16}/></button>}</div>
      ...
      </section>:<section className="running-panel"><h1>{t("正在读取记录")}</h1><button className="secondary" onClick={()=>navigate(null)}>{t("返回首页")}</button></section>}
    </main>
